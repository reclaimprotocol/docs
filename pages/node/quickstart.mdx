...
router.get('/request-proofs', async () => {
  const reclaimProofRequest = new Reclaim.ProofRequest(
    'YOUR_APPLICATION_ID_HERE',
    sessionId
  ) //TODO: replace with your applicationId
  ...
  await reclaimProofRequest.buildProofRequest(providerIds[0])
})
```
...
You can poll the `statusUrl`. When, the proof is uploaded, the `response.session.status` is updated to `MOBILE_SUBMITTED`
