...
If you are integrating it into an app, we recommend using a Frontend SDK, such as [React Native](/react-native) or [Flutter](/flutter).
...
2. **Session Creation**: Your backend uses the Reclaim Node SDK to initiate a session for the user, requesting data from Acme.com. This returns a URI.
3. **Display QR Code**: Your website frontend shows a QR code linked to the session-specific URI returned in the previous step.
4. **QR Code Scan**: The user scans the QR code with their mobile phone regular camera.
