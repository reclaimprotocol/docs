...
Builds proof request for the given providerId. `redirectUser` is an optional parameter that defaults to false. If `redirectUser` is true, the user will be redirected to the website that is set using `setRedirectUrl`, after completing the proof request. `linkingVersion` is an optional parameter that defaults to undefined. If `linkingVersion` contains the value `V2Linking`, the proof request will be created for the new v2 linking flow.
...
function App() {
  ...
  async function generateVerificationRequest() {
    ...
    // buildProofRequest takes providerId, redirectUser, linkingVersion as parameters.
...
<Callout type="info">
  Note: if the `redirectUser` is set to true, `setRedirectUrl` must be called
  before calling `createVerificationRequest`
</Callout>

### Set Redirect URL (Optional, but recommended)
Sets the redirect URL for the user to be redirected to after completing a proof request.
```js copy showLineNumbers {16}
import "./App.css";
import { Reclaim } from "@reclaimprotocol/js-sdk";
...
function App() {
  ...
  async function generateVerificationRequest() {
    ...
    // The user will be redirected to this URL after completing the proof request.
    reclaimClient.setRedirectUrl("https://example.com");
...
<Callout type="info">
  Note that you can use any QR code library you want. Here in the sample we are
  using [react-qr-code](https://www.npmjs.com/package/react-qr-code)
</Callout>
