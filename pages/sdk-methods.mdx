...
#### `verifyProvider(proof, providerHash) ⇒ Promise<boolean>`
...
- `providerHash: string` - Provider hash that you're expecting to be used to generate the proof.

**Returns:**
- `Promise<boolean>` - True if proof's provider is same to the one expected, otherwise false.
<Callout type='info'>
...
We have a section on [how to sign a proof](/js/frontend#set-signature-for-request) for more information.
