...
<p className='text-blue-300 italic'>Reclaim Protocol lets users migrate their profiles from one app to another.</p>
<br></br>

<p className=''>
    Reclaim Protocol has SDKs for frontend, backend and on-chain applications. 
    You can leverage these SDKs to build applications that that imports users' profiles from other websites.

    For example,
    - Import the number of rides the user has taken on Uber to build a competing ride sharing platform 
    - Import the user's purchasing preferences using Amazon to provide discounts to the right users 
    - Enable economics on a blockchain using users' offchain activity that user has performed
    - Use national IDs to perform KYC 
    - And many more...

    We currently support importing data from over 100 websites, aka data providers. If the website you want to import data from isn't on the list, you can build your own data provider using a simple to use [developer tool](https://dev.reclaimprotocol.org).
...
<br></br>
## What does it do?
Using Reclaim Protocol, a user can generate a proof for anything they see on their browser. You can think of these as tamper resistant screenshots. If you take a traditional screenshot of your browser, there is a major drawback - you can easily edit the screenshot making it unusable for any business usecase. Reclaim Protocol lets you take a cryptographic screenshot, using the underlying TLS/HTTPS cryptography that was used to deliver the information to your browser in the first place. So, these proofs can be trusted by businesses to give access and benefits to their users.
...
<Cards>
    ...
    <SdkCard title='React' subtitle='Integrate on your website' link='react' iconName='reactjs' tag='Easiest'/>
    <SdkCard title='React Native' subtitle='Integrate in your mobile app' link='react-native' iconName='react_native' tag="Mobile app"/>
    <SdkCard title='Flutter' subtitle='Integrate in your mobile app' link='flutter' iconName='flutter' tag="Mobile app"/>
