...
The easiest way to handle cancellation is to set up a cancel callback. This allows your application to react immediately when a user's verification session is cancelled.

Read more about setting a cancellation callback in the [JS SDK usage guide](/js-sdk/usage#cancel-callback).

Other errors (such as network timeouts or internal exceptions) may also trigger this callback, and the complete session details can be found by [fetching analytics events](/troubleshooting/analytics-event).
