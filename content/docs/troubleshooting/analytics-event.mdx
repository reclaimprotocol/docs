...
> In case of `ReclaimVerificationAbortedException`, the metadata may contain an additional `has_error` boolean field which will be true if the user cancelled the verification after an error occurred during verification. The exception object may also include a `type` field indicating the specific reason:
>
> | Type | Description |
> |------|-------------|
> | `BROWSER_CONNECTION_FAILED` | The remote browser session failed to start or connect. Retry the verification or contact support if the issue persists. |
