...
description: Find, Build, and Fix providers with your AI agent 
---

If you're building a product that needs to **verify something about your users** — that they own a GitHub account with 1,000+ followers, that their bank balance is above a threshold, that they have an order in their purchase history, or something else — you need a **provider** for it. 
In Reclaim, a provider is the instructions to get data point you want to verify from a website. Each provider has a unique **`providerId`**.
...
- **Find** a provider that already exists,
...
## Step 2 — Find what to verify

Before building anything, check whether a provider already exists. Just ask:

> List my Reclaim providers.

or, to narrow it down:

> Do I already have a provider for GitHub follower count?

Your assistant will look through the providers on your account and show you what's there, including each one's `providerId`. If you find the one you need, you're done — grab the `providerId` and skip to [Step 5](#step-5--put-the-providerid-to-work).

---

## Step 3 — Build a new provider

If nothing exists yet, just describe what you want to verify in plain language:

> Build a Reclaim provider that proves a user's GitHub follower count.
...
4. **Drafts the provider.** It builds a reusable recipe, keeping any private details safe.
...
## Step 4 — Fix a provider that isn't working

Websites change, and a provider that worked last month might stop. When that happens, you don't start over — you ask your assistant to fix it:

> My GitHub follower-count provider stopped working — can you fix it?
...
This is also how you improve a provider (for example, tightening exactly what gets proven) without disrupting anything that already depends on it.
...
- **Reuse across users.** One provider (one `providerId`) works for every user who needs that same proof — build it once, collect proofs forever.
- **Keep providers healthy.** When a site changes, fix the provider in minutes and every integration stays on the same `providerId`.
...
2. **Find** an existing provider, **build** a new one if it's missing, or **fix** one that broke.
