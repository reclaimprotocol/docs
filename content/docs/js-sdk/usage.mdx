...
### More options
  ...
  <Accordion title="Increased Security">
    ...
    Enable AI provider support if your application generates proofs through AI-assisted browser automation rather than direct user interaction.

    ```javascript
    const proofRequestOptions = { acceptAiProviders: true };
    const reclaimProofRequest = await ReclaimProofRequest.init(
        APP_ID,
        APP_SECRET,
        PROVIDER_ID,
    ...
    When `acceptAiProviders` is `true`, the proof request accepts proofs from AI-assisted verification flows. This option is disabled by default.
