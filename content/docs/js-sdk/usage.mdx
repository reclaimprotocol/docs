...
### More options
  ...
  <Accordion title="Increased Security">
    ...
    ### Custom Verification in Session

    Pass an optional `verificationConfig` parameter to `startSession` to customize proof verification. This lets you specify validation requirements instead of using the default provider version.
    ...
    The `verificationConfig` accepts the same options as the second parameter of `verifyProof()`:

    - **Provider information**: Specify `providerId` and optionally `providerVersion` to validate against a specific provider version.
    - **Hash validation**: Pass an array of hashes to validate proofs against specific hashes.
    - **TEE verification**: Set `verifyTEE: true` to verify Trusted Execution Environment attestations.

    Without a `verificationConfig`, the SDK uses the provider version from request initialization.
