...
const { isVerified, data, publicData, error } = await verifyProof(proofs, providerVersion);
if (isVerified) {
...
- **`error`**: Error message if verification failed, undefined otherwise.

Or, by manually providing the provider details:
```javascript
const { isVerified, data, publicData } = await verifyProof(proofs, {
...
```javascript
const { isVerified, data, publicData, error } = await verifyProof(proofs, { hashes: ['0xAbC...'] });
```
