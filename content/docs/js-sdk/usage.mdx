...
### More options
  ...
  <Accordion title="Increased Security">
    ...
    ### TEE attestation
    The remote browser runs in a Trusted Execution Environment (TEE) for privacy. TEE attestation is enabled by default to verify that the session ran in a TEE.
    ...
    To opt out of TEE attestation, set `acceptTeeAttestation: false` during initialization:

    ```javascript
    const proofRequestOptions = { acceptTeeAttestation: false };
    const reclaimProofRequest = await ReclaimProofRequest.init(
        process.env.RECLAIMPROTOCOL_APP_ID,
        process.env.RECLAIMPROTOCOL_APP_SECRET,
        'your-provider-id',
...
### More options
  ...
  The SDK cryptographically binds proofs to a specific session via `verifyProof` and `verifyTeeAttestation`, but it is stateless and cannot detect if the same valid proof has been submitted before. Your server must prevent replay attacks.

  **What the SDK guarantees**

  When you call `verifyProof` with `teeAttestation: { appSecret }`, the SDK:
  - Recomputes the attestation nonce from the proof context using your `appSecret` and verifies it matches—only the holder of the app secret can mint a valid nonce
  - Confirms the nonce appears in the TEE attestation token signed by Google's Confidential Computing OIDC issuer
  - Verifies that `sessionId`, `applicationId`, and `timestamp` in the proof are cryptographically bound

  Users cannot modify `sessionId`, `applicationId`, or `timestamp` on a proof without invalidating it. They can, however, resubmit the same valid proof.

  **What you must do on your server**

  1. **Use a session you initiated.** Record the `sessionId` when you create a proof request. Reject any proof whose `sessionId` you did not issue.
  2. **Dedupe by `sessionId`.** Persist accepted `sessionId` values (e.g., a unique index in your database or a TTL cache) and reject any `sessionId` that has already been verified. This is the primary replay defense.
  3. **Require `teeAttestation`** in `verifyProof` for any flow where replay or tampering matters—without it, the cryptographic binding to your `appSecret` is not checked.
  4. **Enforce a freshness window.** Reject proofs whose `claimData.timestampS` is older than your business policy allows.

  ```javascript
  // Server-side verification with replay protection
  const expectedSessionId = await loadSessionForUser(req.user);
  ...
  if (await db.consumedSessions.has(sessionId)) {
    throw new Error("proof already used"); // replay attempt
  }
  ...
  <Callout type="warn">
  Without server-side session tracking and deduplication, a malicious user can submit the same valid proof repeatedly. The cryptographic checks in `verifyProof` are necessary but not sufficient for replay protection.
  </Callout>
