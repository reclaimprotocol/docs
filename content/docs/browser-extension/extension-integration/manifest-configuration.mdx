...
**`storage`**
- **What**: Access to Chrome's extension storage API
- **Why**: Required for persisting SDK configuration and session state
- **Usage**: SDK stores configuration settings and maintains verification session data
- **Security**: Data is stored locally within the extension's isolated storage area

**`declarativeNetRequest`**
- **What**: Allows modification of network request headers using declarative rules
- **Why**: Required for temporarily modifying Content Security Policy (CSP) headers on provider sites with strict policies
- **Usage**: SDK uses this to enable custom script injection on sites that would otherwise block it due to CSP restrictions
- **Security**: Rules are applied only during active verification flows and are removed immediately after

<Callout type="info">
**Why These Permissions?** The SDK needs to inject code into provider pages, manage session state, and temporarily modify CSP headers on strict sites. This enables the SDK to capture authentication data and generate verification proofs across all provider domains.
</Callout>
...
**Offscreen Document** (`offscreen.html`, `offscreen.bundle.js`, `load-s2circuits.js`)
- Runs WebAssembly for proof generation
- Accessed by the background service worker
- Isolated execution environment
- `load-s2circuits.js` handles loading of STWO cryptographic circuits
...
**Cryptographic Resources** (`browser-rpc/resources/stwo/*`, `browser-rpc/resources/snarkjs/*`)
- STWO and snarkjs circuit files for zero-knowledge proof generation
- Required for the cryptographic operations performed during verification
- Loaded dynamically by the offscreen document when generating proofs
...
- ✅ All required permissions are present: `offscreen`, `cookies`, `scripting`, `storage`, `declarativeNetRequest`
- ✅ Host permissions include required provider domains
- ✅ All SDK resources are listed in `web_accessible_resources` (including STWO and snarkjs resources)
