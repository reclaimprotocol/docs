...
- `options` (object): Configuration options
  ...
  - `logConfig` (object): Optional - Logging configuration (see [Logging Configuration](#logging-configuration))
...
The SDK includes built-in logging that can be configured for debugging. Use the `setLogConfig()` method or pass `logConfig` in initialization options:
...
const configApplied = await reclaimExtensionSDK.setLogConfig(
  {
    logLevel: "DEBUG",
    consoleEnabled: true,
  },
...
if (!configApplied) {
  console.warn("Log config could not be applied");
}
```

You can also listen to SDK events for additional debugging:
...
See [Logging Configuration](#logging-configuration) for more details.

## Logging Configuration

The SDK provides methods to configure logging for debugging and diagnostics.

### `reclaimExtensionSDK.setLogConfig()`

Configure the SDK's logging behavior. This method is asynchronous and returns a promise indicating whether the configuration was applied.

```javascript
const configApplied = await reclaimExtensionSDK.setLogConfig(config, extensionID, timeout);
```

**Parameters**:

- `config` (object): Logging configuration
  - `logLevel` (string): Log level - `"INFO"` (default) or `"DEBUG"` for verbose output
  - `consoleEnabled` (boolean): Whether to output logs to the browser console
- `extensionID` (string): Required in web mode - the browser extension ID
- `timeout` (number): Optional timeout in milliseconds (default: 2000)

**Returns**: `Promise<boolean>` - Resolves to `true` if configuration was applied, `false` on timeout

**Example**:

```javascript
// Enable debug logging with console output
const success = await reclaimExtensionSDK.setLogConfig(
  {
    logLevel: "DEBUG",
    consoleEnabled: true,
  },
...
if (success) {
  ...
  console.warn("Could not enable logging - extension may not be ready");
}
```

---

### `reclaimExtensionSDK.getLogConfig()`

Retrieve the current logging configuration.

```javascript
const config = await reclaimExtensionSDK.getLogConfig();
```

**Parameters**: None

**Returns**: `Promise<Object>` - Resolves to the current log configuration:
...
```javascript
const currentConfig = await reclaimExtensionSDK.getLogConfig();
console.log("Current log level:", currentConfig.logLevel);
console.log("Console logging:", currentConfig.consoleEnabled ? "enabled" : "disabled");
```
...
| Level   | Description                                        |
| ------- | -------------------------------------------------- |
| `INFO`  | Default level - logs important events and errors   |
| `DEBUG` | Verbose logging - includes detailed diagnostic info |
