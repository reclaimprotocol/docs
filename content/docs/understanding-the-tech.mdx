...
1. **TEE_K and TEE_A** — These ensure integrity and privacy during verification. They guarantee that verification data can't be tampered with and that sensitive information stays protected throughout the flow.

2. **TEE_REMOTE_BROWSER** — This ensures that the remote browser used for verification doesn't collect or leak any user data. It's only used in the [JS SDK](/js-sdk/usage) for remote browser verification. The [InApp SDKs](/inapp-sdks/introduction) use the device's local in-app browser, so this component doesn't apply to mobile integrations.
