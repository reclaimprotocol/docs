    ...
    responseRedactions: [
      {
        ...
        hash: 'oprf', // Hash type for redacted data (optional)
      }
    ]
  }
```

### Hash Types for Redactions

When using `responseRedactions`, you can specify a `hash` type to control how redacted data is processed. Available options:

| Hash Type | Description |
|-----------|-------------|
| `oprf` | Oblivious Pseudo-Random Function. Default hash type. |
| `oprf-mpc` | Multi-Party Computation variant of OPRF. Distributes the hashing across multiple parties for enhanced privacy. |

Defaults to `oprf` if not specified.
