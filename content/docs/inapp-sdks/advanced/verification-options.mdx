...
| **`useTeeOperator`**                           | <code>boolean \| nil</code>            | Controls TEE (Trusted Execution Environment) mode. Set to `true` to enable TEE, `false` to disable, or `nil` (default) for automatic selection based on provider configuration. |
