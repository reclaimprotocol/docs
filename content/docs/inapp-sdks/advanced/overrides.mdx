...
model HttpProvider {
  ...
  # Specifications for injected requests that the SDK can make on behalf of the user.
...
# Specifications for injected requests that can be made by the SDK.
# Unlike intercepted requests (RequestData), injected requests are initiated
# by the SDK rather than intercepted from user browser activity.
type InjectedRequestSpec {
  # The URL for the HTTP request. Supports template variables using {{variable}} syntax.
  # If parameter name is suffixed with _GRD, greedy matching is used; otherwise lazy.
  url                String
  # Type of URL: CONSTANT for exact match, TEMPLATE for pattern matching.
  urlType            urlType
  # HTTP method (GET, POST, PUT, PATCH, DELETE).
  method             RequestMethodType
  # Response validation rules. At least one match must be provided.
  responseMatches    ResponseMatch[]
  # Redaction rules for sensitive response data.
  responseRedactions ResponseRedaction[]
  # Body capturing configuration.
  bodySniff          BodySniff?
  # Template parameter variables for dynamic request construction.
  templateParams     String[]
  # Whether this request is required in the list of proofs. Defaults to true.
  required           Boolean             @default(true)
  # Whether this request can appear multiple times in proofs. Defaults to true.
...
type ResponseMatch {
  # The pattern or value to match. Supports template variables using {{variable}} syntax
  # when type is 'contains'. For 'regex' type, can extract variables using named groups.
  value       String
  # Matching mechanism: 'regex' for regular expressions, 'contains' for substring match.
  type        ResponseMatchType
  # If true, the match condition is reversed. Defaults to false.
  invert      Boolean             @default(false)
  # If true, this match is optional and won't fail if absent. Defaults to false.
...
type ResponseRedaction {
  ...
  jsonPath String?
  # Regular expression for parsing and extracting/redacting values.
  # Can extract fields using named regex groups.
  regex    String?
  # XPath for XML/HTML matching configuration.
  xPath    String?
  # Hashing method applied to redacted content.
  hash     RedactionHashType?
}

enum RedactionHashType {
  # OPRF (Oblivious Pseudorandom Function) - works only for named regex groups.
  oprf
  # OPRF with multi-party computation.
  oprf_mpc
  # Raw OPRF output.
  oprf_raw
}
...
The provider schema supports two request types:

- **`requestData`** (Intercepted Requests): Captured from the user's browser activity. The SDK monitors network traffic and captures requests matching specified patterns. Use this to verify data the user naturally accesses during their session.

- **`allowedInjectedRequestData`** (Injected Requests): Initiated by the SDK on behalf of the user. The SDK makes these HTTP requests directly instead of waiting for browser activity. Injected requests support additional options like `templateParams` for dynamic URL construction, and `required`/`multiple` flags to control proof requirements.
...
- For use with `jsonString`, provide the value of the 'providers' key in the `jsonString` parameter.
