...
* **Verification Portal**: Added client authentication support for session creation and provider verification. The Developer Portal backend now generates JWT tokens (HS256, 5-minute expiry) for client authorization when authentication credentials are configured, passing them automatically to verification requests. This enables multi-tenant credential management with automatic fallback to default credentials when authentication is unavailable.
